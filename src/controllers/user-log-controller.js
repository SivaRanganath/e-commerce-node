...
const { validateUserSignUpData, validateKeysInBody } = require("../utils/user-validation");
...
exports.signUpuser = async (req, res) => {
    try {
        ...
        const isValidBody = validateKeysInBody(req, UserModel.schema.obj);
        if (!isValidBody) {
            res.status(400);
        ...
        if (isUserDataErrors.length > 0) {
            res.status(400);
            return res.send({ message: "Validation Errors", errors: isUserDataErrors });
        }
